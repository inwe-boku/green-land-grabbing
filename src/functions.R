...
get_intersecting_municipalities <- function(municipalities, wind_parks, pv_parks) {
  ...
  intersecting_municipalities_wind <-
    st_intersects(municipalities, wind_parks, sparse = FALSE)
  intersecting_municipalities_pv <-
    st_intersects(municipalities, pv_parks, sparse = FALSE)
  intersecting_wind_municipalities <-
    which(rowSums(intersecting_municipalities_wind) > 0)
  intersecting_pv_municipalities <-
    which(rowSums(intersecting_municipalities_pv) > 0)
  
  municipalities_intersecting_total <-
    municipalities[union(intersecting_wind_municipalities,
                         intersecting_pv_municipalities),]
...
load_wind_data <- function() {
  ...
  join_bloomberg_aneel_owners_investors <-
    get_windparks_aneel_bloomberg_owners_investors()
  
  join_bloomberg_aneel_owners <-
    join_bloomberg_aneel_owners_investors[[1]]
  join_bloomberg_aneel_investors <-
    join_bloomberg_aneel_owners_investors[[2]]
  
  organizations_unique <- get_organizations_windparks()
  
  get_full_owners_investors_aneel_windparks(
    join_bloomberg_aneel_owners,
    join_bloomberg_aneel_investors,
...
load_pv_data <- function() {
  ...
  join_bloomberg_aneel_owners_investors <-
    get_solar_aneel_bloomberg_owners_investors()
  ...
  joined_data <-
    get_full_owners_investors_aneel_solarparks(
      join_bloomberg_aneel_owners,
      join_bloomberg_aneel_investors,
      organizations_unique
    )
...
load_areas_pv <- function() {
  ...
  pv_areas <-
    st_read("data/solar-pv_area-manual-area_BR_14-06-2022.shp")
...
load_aneel_pv <- function() {
  ...
  pv_aneel <-
    st_read("data/solar-pv_operating_min-5MW_UFV_BR_edit-aneel_04-02-2022.shp")
...
derive_shape_file_pv <- function(areas_pv, aneel_pv, joined_data_pv) {
  ...
  areas_pv %>%
    ...
    full_join(
      aneel_pv %>% as_tibble() %>% mutate(AREA.ID = as.numeric(AREA.ID)) ,
...
write_ownership_investors_by_country_region <- function(joined_data,
  ...
  joined_data_region %>%
    ...
    mutate(area_sum = sum(area)) %>%
    ungroup() %>%
    group_by(Country_Category, Type) %>% summarize(a = 100 * sum(area) /
                                                     max(area_sum)) %>%
...
shorten_company_name <- function(company,
                                 country,
                                 length_tot = 40,
                                 linebreak = 20) {
...
do_sankey <- function(joined_data,
                      type,
                      nmb_companies,
                      colormap = "D",
                      title = "",
  ...
  joined_data <- joined_data %>%
    mutate(Country_Category = ifelse(
      Country_Category == "Other",
      "Other regions",
      Country_Category
    )) %>%
    mutate(
      parent_Country_Category = ifelse(
        parent_Country_Category == "Other",
        "Other regions",
        parent_Country_Category
      )
    )
  ...
  joined_data_cat <- joined_data %>%
    ...
    summarize(area = sum(area)) %>%
    arrange(area) %>%
    mutate(parent_Country_Category = ifelse(
      is.na(parent_Country_Category),
      "No information",
      parent_Country_Category
    ))
  
  level2 <- joined_data_cat %>%
    dplyr::select(source = Country_Category,
                  target = single,
                  value = area) %>%
  ...
  level3 <- joined_data_cat %>%
    mutate(
      company_info_act_parent = ifelse(
        single == "Other companies",
        "Other companies",
        company_info_act_parent
      )
    ) %>%
    mutate(
      company_info_act_parent = ifelse(
        single == company_info_act_parent,
        glue("{single}"),
        company_info_act_parent
      )
    ) %>%
    dplyr::select(source = single,
                  target = company_info_act_parent,
                  value = area)  %>%
  ...
  level4 <- joined_data_cat %>%
    ...
    dplyr::select(source = company_info_act_parent,
                  target = parent_Country_Category,
                  value = area)  %>%
  ...
  source_data <- joined_data_region %>%
    ...
    #na.omit() %>%
    dplyr::select(
      `Total park area` = source,
      `Direct country` = Country_Category,
      value = area
    ) %>%
...
get_full_owners_investors_aneel_solarparks<-function(join_bloomberg_aneel_owners,
  ...
  direct_owner <- join_bloomberg_aneel_owners %>%
    left_join(organizations_unique,
              by = c("Direct Owner Tickers" = "Bloomberg Ticker")) %>%
    ...
    dplyr::select(
      ...
      Key,
      company_info,
      company_name = `Organization Name Matched`,
      ownership_p,
      date_close,
      company_info_act
    )
  ...
  direct_investor <- join_bloomberg_aneel_investors %>%
    left_join(organizations_unique,
              by = c("org_name" = "Organization Name Matched")) %>%
  ...
  joined_data <- bind_rows(direct_owner,
    ...
    mutate(Country_Category = ifelse(
      Country_Category %in% c("Canada", "United States"),
      "North America",
      Country_Category
    )) %>%
    mutate(Country_Category = ifelse(
...
get_full_owners_investors_aneel_solarparks_including_parents<-function(join_bloomberg_aneel_owners,
  ...
  direct_owner <- join_bloomberg_aneel_owners %>%
    ...
    mutate(company_info_act = `Organization Name Matched`) %>%
    left_join(
      organizations_unique %>% dplyr::select(
        `Bloomberg Ticker`,
    ...
    dplyr::select(
      ...
      Key,
      company_info,
      company_name = `Organization Name Matched.x`,
      ownership_p,
      date_close,
      company_info_act,
      company_info_parent,
  ...
  joined_data <- bind_rows(direct_owner,
    ...
    mutate(
      parent_Country_Category = ifelse(
        parent_country %in% c("Canada", "United States"),
        "North America",
        parent_Country_Category
      )
    ) %>%
...
load_aneel_wind <- function() {
  ...
  wind_parks_aneel_shape <-
    st_read(
      "data/wind-parks_JOIN_turbines_power-plants_EOL_BR_aneel_10_05_2023_collected_encoded_fin.shp"
    )  %>%
...
get_organizations_windparks <- function() {
  ...
  organizations <-
    read_xlsx("data/organizations_A-Z_2022_matched.xlsx") %>%
    bind_rows(
      tibble(
        `Organization Name Matched` = c(
          "Forca Eolica Do Brasil",
          "MSU Energy SA",
          "Kingdom of Denmark",
        ...
        `Parent/Operational Investor` = c(
          "Neoenergia Renovaveis SA",
          "MSU Energy SA",
          "Kingdom of Denmark",
          "Government of Brazil"
        )
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Cia Paranaense de Energia",
        "Government of Brazil",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Petroleo Brasileiro SA",
        "Government of Brazil",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Superintendencia do Desenvolvimento do Nordeste",
        "Government of Brazil",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        startsWith(`Organization Name Matched`, "Fundacao Ele"),
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Neoenergia Renovaveis SA",
        "Iberdrola SA",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Neoenergia SA",
        "Iberdrola SA",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "PEC Energia SA",
        "Iberdrola SA",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Servtec Energia Ltd",
        "Grupo HLC Sgps",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Horizonte Energias Renovaveis Ltda",
  ...
  organizations %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Gestamp Eolica Brasil SA",
        1,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Sowitec do Brasil Energias Alternativas Ltda",
        3,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Fundacao Eletrosul de Previdencia e Assistencia Social",
        4,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Renobrax Energias Renovaveis",
        6,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Sequoia Capital Ltda",
        7,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Ventos Brasil Comercio e Representacoes Ltda",
        10,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Martifer Renewables SGPS SA",
        11,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Grupo HLC Sgps",
        12,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Rio Sul 1 Energia Ltda",
        13,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "State Grid Brazil Power Participacoes SA",
        14,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "SoWiTec International GmbH",
        15,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` %in% c(
          "Actis LLP",
          "Banco Santander SA",
          "BNP Paribas SA",
          "Brazil Energy SA",
...
get_windparks_aneel_bloomberg_owners_investors <- function() {
  ...
  wind_parks_bloomberg_all_transactions <-
    clean_bloomberg_names(full_data_bloomberg)
  ...
  wind_parks_bloomberg_all_transactions_owners <-
    wind_parks_bloomberg_all_transactions %>% mutate(n = 1:n()) %>%  group_by(Key, `Direct Owner Tickers`) %>%
    filter(n == min(n))
  ...
  owners <- wind_parks_aneel %>%
    ...
    dplyr::select(
      ...
      INICIO_OPE,
      AREA_M2,
      `Direct Owner Tickers`,
      `Parent Owner Tickers`,
      Start_year,
      `% of ownership`,
      org_name,
      date_close
  ...
  investors <- wind_parks_aneel %>%
    ...
    mutate(share = ifelse(
      eq_provid &
        debt,
      (1 - R_PV) * share_int,
      ifelse(eq_provid, share_int, share)
    )) %>%
    ...
    mutate(
      org_name = ifelse(
        org_name == "Echoenergia e Participacoes SA",
        "Echoenergia Participacoes SA",
        org_name
      )
    ) %>%
    mutate(
      org_name = ifelse(
        org_name == "Enernova-Novas Energias SA",
        "Enernova Novas Energias SA",
        org_name
      )
    ) %>%
    mutate(org_name = ifelse(
      org_name == "Kirton Bank SA Banco Multiplo",
      "Kirton Bank SA",
      org_name
    )) %>%
    mutate(
      org_name = ifelse(
        org_name == "Petra Asset Gestao de Investimentos Ltda/Brazil",
        "Petra Asset Gestao de Investimentos Ltda",
        org_name
      )
    ) %>%
    ...
    mutate(org_name = ifelse(
      org_name == "Ventos do Sul Energia SA",
      "Ventos do Sul Energia S/A",
      org_name
    )) %>%
    mutate(org_name = ifelse(
      org_name == "Neoenergia SA",
      "Neoenergia Renovaveis SA",
      org_name
    ))
...
get_solar_aneel_bloomberg_owners_investors<-function() {
  ...
  solar_parks_bloomberg_all_transactions <-
    clean_bloomberg_names(full_data_bloomberg, "Solar")
  
  solar_parks_bloomberg <-
    solar_parks_bloomberg_all_transactions %>%
    filter(`Capacity (MW)` > 5) %>%
    dplyr::select(Name, Key, Bloomberg_Name = `Project name`, geometry) %>%
    unique()
  
  solar_parks_bloomberg_full <-
    solar_parks_bloomberg_all_transactions %>%
    filter(`Capacity (MW)` > 5) %>%
    dplyr::select(
      Name,
      Key,
      Bloomberg_Name = `Project name`,
      abstract,
      descriptio,
      prj_abstra,
      prj_descri,
  ...
  solar_parks_aneel <-
    clean_aneel_names(solar_parks_aneel %>% mutate(NOME_EOL = NOME))
  
  solar_parks_aneel_areas <-
    st_read("data/solar-pv_area-manual-area_BR_14-06-2022.shp")
  ...
  solar_parks_bloomberg_all_transactions_owners <-
    solar_parks_bloomberg_all_transactions %>%
    ungroup() %>%
    mutate(n = 1:n()) %>%
  ...
  join_bloomberg_aneel_investors <- solar_parks_aneel_area_join %>%
    ...
    mutate(
      org_name = ifelse(
        org_name == "Canadian Solar Brasil Comercializacao Importacao e Exportacao de Paineis Solare",
        "Canadian Solar Brasil Comercializacao Importacao e Exportacao de Paineis Solares",
        org_name
      )
    ) %>%
    ...
    mutate(
      org_name = ifelse(
        org_name == "Solatio Energia",
        "Solatio Brasil Gestao de Projetos Solares Ltda",
        org_name
      )
    ) %>%
    mutate(
      org_name = ifelse(
        org_name == "Sunedison Brasil",
        "SunEdison Brasil - Projetos Montagem e Instalacao de Empreendimentos de Energia",
        org_name
      )
    )
...
get_organizations_solarparks<-function() {
  ...
  organizations <-
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Cia Energetica de Minas Gerais",
        "Government of Minas Gerais",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "EDP - Energias do Brasil SA",
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Inter-American Investment Corp",
    ...
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Mori Energia",
        "Grupo Perfin",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Rio Alto Energia Empreendimentos e Participacoes Ltda",
        "Grupo Rio Alto",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Solatio Energia",
        "Solatio",
        `Parent/Operational Investor`
      )
    ) %>%
    mutate(
      `Parent/Operational Investor` = ifelse(
        `Organization Name Matched` == "Solatio Brasil Gestao de Projetos Solares Ltda",
        "Solatio",
        `Parent/Operational Investor`
      )
    )
  ...
  organizations_unique <- organizations %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Canadian Solar Brasil Comercializacao Importacao e Exportacao de Paineis Solares",
        1,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Fram Capital Distribuidora de Titulos e Valores Mobiliarios SA",
        2,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "KFW Development Bank",
        3,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Sertao Brasil Energia Solar Eireli - ME",
        4,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Sindustrial Engenharia Ltda",
        5,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Steelcons Empreiteira Construcao Civil Ltda",
        7,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "SunEdison Brasil - Projetos Montagem e Instalacao de Empreendimentos de Energia",
        8,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Total Eren SA",
        9,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Verde Vale Energia Ltda",
        10,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Vila Energia Renovavel SS Ltda",
        11,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "SESA Bidco Ltd",
        12,
        `Bloomberg Ticker`
      )
    ) %>%
    ...
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "Mori Energia Solar",
        14,
        `Bloomberg Ticker`
      )
    ) %>%
    mutate(
      `Bloomberg Ticker` = ifelse(
        `Organization Name Matched` == "D2C Energias Renovavies e Participacoes Ltda",
        15,
        `Bloomberg Ticker`
      )
    ) %>%
...
clean_bloomberg_names<-function(full_data, Technology = "Wind") {
  ...
  full_data_selection <- full_data %>%
    ...
    mutate(Key = str_replace(Key, "Martifer", ""))  %>%
    mutate(Key = ifelse(
      str_detect(Key, "SerraDaBabiloniaE"),
      "SerradaBabiloniaE",
      Key
    )) %>%
    ...
    mutate(Key = ifelse(
      Key == "RioEnergy SerraDaBabiloniaE",
      "SerradaBabiloniaE",
      Key
    )) %>%
  ...
  if (Technology == "Wind") {
    ...
    full_data_selection <- full_data_selection %>%
      ...
      mutate(
        Key = ifelse(
          Key == "SoldoFuturoII",
          "SteelconsSoldoFuturoII(AntigaSteelconsMiracema2)",
          Key
        )
      ) %>%
      mutate(
        Key = ifelse(
          Key == "SoldoFuturoIII",
          "SteelconsSoldoFuturoIII(AntigaSteelconsMiracema3)",
          Key
        )
      ) %>%
...
load_bloomberg_data<-function() {
  ...
  wind_solar <-
    st_read("data/wind_solar_BR_bloomberg_2000-2021_31-01-2022.shp")
  wind_solar1 <-
    read_xlsx("data/wind_solar_2000_2021_updated.xlsx", sheet = 1)
  wind_solar$project_status <- wind_solar1$`Project >> Status`
  wind_owners <-
    read_xlsx("data/wind_solar_2000_2021_updated.xlsx", sheet = 3) %>%
    mutate(Type = "Wind")
  solar_owners <-
    read_xlsx("data/wind_solar_2000_2021_updated.xlsx", sheet = 4) %>%
    mutate(Type = "Solar")
