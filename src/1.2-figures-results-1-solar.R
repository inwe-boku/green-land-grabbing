...
write_ownership_investors_by_country_region(
  joined_data_pv,
...
joined_data_region_full_pv <-
  aggregate_data_by_region_full(joined_data_region_pv)
...
shape_file_pv <-
  derive_shape_file_pv(areas_pv, aneel_pv, joined_data_pv)
...
joined_data_pv <-
  get_full_owners_investors_aneel_solarparks_including_parents(
    join_bloomberg_aneel_owners,
    join_bloomberg_aneel_investors,
...
joined_data_pv_ <- joined_data_pv %>%
  ...
  mutate(company_info_act_parent = abbreviate(company_info_act_parent, minlength =
                                                30)) %>%
  mutate()


p <-
  do_sankey(title = "Owner", joined_data_pv_, "Direct owner", 6, "I",-1, 0.7, 0.3, 1)
p
ggsave("figures/sankey-pv-owner.eps",
       p,
       width = 12,
       height = 6)
ggsave("figures/sankey-pv-owner.pdf",
       p,
       width = 12,
       height = 6)
ggsave("figures/sankey-pv-owner.png",
       p,
       width = 12,
       height = 6)
...
p <-
  do_sankey(title = "Investor", joined_data_pv_, "Direct investor", 6, "I",-1, 0.7, 0.3, 1)
p
ggsave("figures/sankey-pv-investor.eps",
       p,
       width = 12,
       height = 6)
ggsave("figures/sankey-pv-investor.pdf",
       p,
       width = 12,
       height = 6)
ggsave("figures/sankey-pv-investor.png",
       p,
       width = 12,
       height = 6)
...
has_international_involvment <- joined_data_pv_ %>%
  mutate(
    international_involvment = ifelse(
      Country_Category != "Brazil" |
        parent_Country_Category != "Brazil",
      1,
      0
    )
  ) %>%
  group_by(Key) %>%
  summarize(international_involvment = max(international_involvment, na.rm =
                                             TRUE))
