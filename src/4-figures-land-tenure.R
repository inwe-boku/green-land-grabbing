...
wind_turbines_analysis <-
  read_csv("intermediate/wind_turbines_analysis.csv")

wind_turbines_analysis %>%
  mutate(count_share = count / sum(count))

all_intersections_wind <-
  read_csv("intermediate/all-intersections-wind.csv") %>%
...
all_intersections_pv <-
  read_csv("intermediate/all-intersections-pv.csv") %>%
...
all_intersections <-
  bind_rows(
    all_intersections_wind %>% mutate(tech = " Wind power"),
...
plot_intersections_panel <- function(all_intersections, add_string) {
  ...
  dat_mean$Area = factor(dat_mean$Area,
                         levels = c("Park",
                                    "Control random",
  ...
  ggsave("figures/land-tenure-panel-mean.pdf",
         p,
         width = 10,
         height = 6)
...
plot_intersections_complete <-
  function(all_intersections, add_string) {
    ### add land share
    ...
    ggsave(
      glue("figures/land-tenure-{add_string}-mean.png"),
      p,
      width = 10,
      height = 6
    )
    
    all_intersections_area_share$Area = factor(
      all_intersections_area_share$Area,
      levels = c("Park",
                 "Control random",
                 "Control match wind resource")
    )
    ...
    ggsave(
      glue("figures/land-tenure-{add_string}-boxplots.png"),
      p,
      width = 10,
      height = 6
    )
    ...
    if (add_string == "wind") {
      return(
        all_intersections_area_share %>%
          ungroup() %>%
          spread(Area, area_share) %>%
          group_by(category) %>%
          summarize(
            p.value.observed.match.wind.5p = t.test(`Park`,  `Control match wind resource`)$p.value,
            ...
            t.statistic.observed.match.wind.5p = t.test(`Park`,  `Control match wind resource`)$statistic,
            t.statistic.observed.random.5p = t.test(`Park`,  `Control random`)$statistic,
            t.statistic.random.match.wind.5p = t.test(`Control random`,  `Control match wind resource`)$statistic
          )
      )
    } else{
      return(
...
res <-
  plot_intersections_complete(all_intersections_wind, "wind") %>%
  mutate(tech = "Wind")

res1 <- plot_intersections_complete(all_intersections_pv, "pv") %>%
  mutate(tech = "Solar")

res %>% gather(variable, value,-category,-tech) %>%
  bind_rows(res1 %>% gather(variable, value,-category,-tech)) %>%
  mutate(value = round(value, 4)) %>%
  mutate(category = str_replace_all(category, "[\r\n]", " ")) %>%
  mutate(category = str_replace_all(category, " Private", "Private")) %>%
  write_csv("intermediate/t.tests.csv")
...
summary(lm(
  area_km2_share ~ power_density,
  data = privatizations_park %>%
...
plot_cumulative_privatizations <- function(all_intersections) {
  privatizations <- all_intersections %>%
    ...
    ungroup() %>%
    filter(category == " Private land with\nlegal property title") %>%
  ...
  privatizations_small$Area = factor(
    privatizations_small$Area,
    levels = c("Park",
               "Control random",
               "Control match wind resource")
  )
  ...
  ggsave(
    paste0("figures/cumulative-privatizations-over-time.png"),
    width = 10,
    height = 6
  )
