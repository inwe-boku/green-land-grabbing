...
land_tenure <-
  st_read("data/land-tenure/Landtenuremodel/landtenuremodel.shp")
...
wind_parks <-
  st_read("intermediate/wind_parks_power_density.shp") %>%
  st_transform(crs_all)

p_random <-
  st_read("intermediate/random-shapes/whole-area-wind-1.shp") %>%
  st_transform(crs_all)
...
all_parks <- st_buffer(all_parks, dist=0)
